...
**SOPS** is an editor of encrypted files that supports YAML, JSON, ENV, INI, PXF
(protowire) and BINARY formats and encrypts with AWS KMS, GCP KMS, Azure Key Vault,
...
YAML, JSON, ENV, INI and PXF type extensions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

SOPS uses the file extension to decide which encryption method to use on the file
content. ``YAML``, ``JSON``, ``ENV``, ``INI``, and ``PXF`` files are treated as trees of data, and key/values are
...
PXF (protowire text format)
~~~~~~~~~~~~~~~~~~~~~~~~~~~

``PXF`` (Protocol-buffer eXchange Format) is a typed, comment-aware text
format from the `trendvidia/protowire <https://github.com/trendvidia/protowire>`_
project. SOPS recognises it via the ``.pxf`` extension or by passing
``--input-type protowire`` / ``--output-type protowire`` (``pxf`` is also
accepted as an alias).

Compared to YAML or JSON, PXF carries explicit value types in the syntax —
strings are quoted, integers and floats are bare numerals, booleans use
``true``/``false``, ``null`` is a literal, byte literals use ``b"..."``,
RFC 3339 timestamps and Go-style durations are recognised, lists use
``[ ... ]``, and nested objects use ``{ ... }`` blocks. As a result, a
SOPS-decrypted PXF file always restores the exact value type of each leaf
without the JSON/YAML guesswork around quoting.
...
Two format constraints worth knowing:

1. **Top-level keys must be identifiers.** PXF's grammar only allows
   identifier-shaped names (letters, digits, ``_``, ``.``) at the top
   level of a document, so a top-level key like ``"Welcome!"`` is not
   accepted. Inside a nested ``{ ... }`` block any string key is fine.
2. **No mixed top-level documents.** A SOPS PXF file always stores a
   single message at the top level (the same constraint that JSON's
   top-level object enforces). YAML-style multi-document streams are
   not supported.

Comments use ``#`` or ``//``, are preserved through encrypt/decrypt, and
participate fully in the ``encrypted_comment_regex`` /
``unencrypted_comment_regex`` features as well as the
``# sops:unencrypted`` / ``# sops:encrypted`` directives described below.
...
JSON, PXF, and TEXT file types do not support anchors and thus have no such limitation.
...
Top-level arrays
~~~~~~~~~~~~~~~~
``YAML``, ``JSON``, and ``PXF`` top-level arrays are not supported, because SOPS
needs a top-level ``sops`` key to store its metadata. (PXF goes further: even
non-array top-level data must use identifier-shaped keys — see the PXF section
above.)
...
In addition to the suffix and regex rules above, individual fields can be
flagged with a comment-based directive. A directive applies to the next
non-comment item *and to all of its descendants*, and overrides any
suffix / regex rule that would otherwise apply. The directive comment
itself is never encrypted, so it round-trips through encrypt/decrypt.

* ``# sops:unencrypted`` — leave the next item (and its subtree) in clear text.
* ``# sops:encrypted`` — force-encrypt the next item (and its subtree).

The match is case-insensitive and tolerates leading or trailing
whitespace, so ``# sops:unencrypted``, ``// SOPS:UNENCRYPTED`` and
``#sops:unencrypted`` are all recognised. Directives only work in formats
that preserve comments (``yaml``, ``protowire``/``pxf``, ``ini``); they
have no effect in ``json`` or ``dotenv``, which discard comments.
...
.. code:: text
   ...
   # sops:unencrypted
   public_config {
     host = "example.com"  # stays in plaintext
     port = 8080           # stays in plaintext
   }

   secret = "still encrypted"

The same directive in YAML:
...
Directives can mix freely with normal encryption inside the same block, which
is the main advantage over the ``unencrypted_suffix`` rule — the affected key
keeps its name and you only opt out of encryption for the specific fields you
care about:
...
.. code:: text
   ...
   database {
     host     = "db.internal:5432"
     password = "p@ssw0rd!"            # encrypted
     # sops:unencrypted
     pool_size = 32                    # plaintext
     # sops:unencrypted
     read_replicas = [                 # plaintext
       "replica-a.internal:5432",
       "replica-b.internal:5432"
     ]
   }

Precedence relative to the other rules: directives are evaluated **last** and
override any suffix, regex, or comment-regex match for the affected item.
When directives are nested, the innermost directive in scope wins; when two
directives appear on consecutive lines before the same item, the later one
wins.
